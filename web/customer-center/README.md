...
```
https://<host>/center/<slug>
```

No URL token. Each user signs in with **email + password** — credentials are stored in the per-customer `kai-<slug>-users` Secret (argon2id-hashed). The same `kai-session` cookie is honored by `customer-chat` when both apps share an origin (single-sign-on).
...
- **First visit on a fresh customer** — the `users.json` is empty, so the login form switches to "Set up your customer center". The first email + password to submit becomes the initial admin. After that, the bootstrap path is closed (the second submission goes through normal sign-in).
...
Per-customer team list and the customer-facing distillations of `SOUL.md` / `HEARTBEAT.md` live in a single ConfigMap. The operator's onboarding flow can populate this from the equivalent files in `swarm-config/customers/<slug>/`:
...
npm install
npm run dev   # http://localhost:3004/center/<slug>
```
...
```bash
docker build -t emai-customer-center:latest .
```
...
kubectl port-forward -n emai-swarm svc/customer-center 8080:8080
```

K8s manifests live at [`../../kubernetes/customer-center/`](../../kubernetes/customer-center/).
...
|--------|-------------------------------------------------------|---------------------|--------------------------------------------------------------------------|
| GET    | `/healthz`                                            | none                | `ok`                                                                     |
| GET    | `/api/center/{slug}/auth`                             | none                | `{authenticated, email?, slug?, needsSetup}` — drives the login UI        |
| POST   | `/api/center/{slug}/login`                            | email + password    | Sets `kai-session` cookie. Bootstraps when `users.json == []`.           |
| POST   | `/api/center/{slug}/logout`                           | cookie              | Clears the cookie.                                                       |
| GET    | `/api/center/{slug}`                                  | cookie              | `{customerName, projectName, slug, status, statusLabel, links[], channels[], team[], scope, heartbeat, briefings[]}` |
| GET    | `/api/center/{slug}/users`                            | cookie              | `[{email, createdAt, passwordUpdatedAt}]` — no hashes ever exposed       |
| POST   | `/api/center/{slug}/users`                            | cookie              | Add `{email, password}` — server hashes argon2id                         |
| DELETE | `/api/center/{slug}/users/{email}`                    | cookie              | Remove user                                                              |
| POST   | `/api/center/{slug}/users/{email}/password`           | cookie              | Reset `{password}`                                                       |
...
- **Same cookie + same secret as `customer-chat`** — when both apps are served from one origin (production ingress), signing in once works for both.
