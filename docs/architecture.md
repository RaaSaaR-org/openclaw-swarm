...
| **Chat** (`web/chat/`) | Vite SPA + Go server | End user | Per-tenant JWT cookie | Tenant-facing chat UI. Logs the user in against a per-tenant users Secret, then bridges WebSocket traffic to the OpenClaw pod's gateway. |
| **Workspace** (`web/workspace/`) | Vite SPA + Go server | Tenant admin | Per-tenant JWT cookie + bootstrap-admin first-login | Tenant dashboard: manage Team (add/remove users + reset passwords), see briefings, see profile (scope, heartbeat). Same JWT cookie as chat — login on one is honored by the other when they share an origin. |
...
| **OpenClaw pod** (`ghcr.io/openclaw/openclaw:latest`) | Node.js (third-party) | (no direct humans — through chat) | Gateway token + control-UI bypass for chat | The actual AI agent. One pod per tenant. Reads its workspace from the per-tenant ConfigMap and PVC. |

The **shared library `pkg/auth/`** holds JWT + argon2id helpers used by chat and workspace. It exists so the two services can't drift on auth semantics — see TASK-004 for the rationale.
...
| **Per-tenant JWT** | chat, workspace | `kai-<slug>-chat-bridge` Secret (`jwt-secret` key, 32 random bytes generated by the operator) | 24h cookie TTL | One slug only — the JWT carries `slug` in its claims and is rejected against any other slug |
...
- The bootstrap-admin flow lives in workspace: when `kai-<slug>-users` is empty, the first POST to `/api/workspace/<slug>/login` is treated as initial setup — the submitter's email + password becomes the admin record. After that, normal login + Team-page-driven user CRUD.
...
| Chat | `kai-<slug>-chat-bridge` (jwt-secret + device keys for the WS handshake); `kai-<slug>-users` (verify password) | (none — read-only against K8s) | `secrets[get,list]` in the namespace |
| Workspace | `kai-<slug>-chat-bridge` (jwt-secret); `kai-<slug>-users`; tenant-profile ConfigMaps (`kai-<slug>-briefings`, scope, heartbeat) | `kai-<slug>-users` (add/remove users, reset password) | `secrets[get,list,update]` + `configmaps[get,list]` in the namespace |
...
The chat **WebSocket bridge** then dials the per-tenant OpenClaw gateway over the in-cluster Service `kai-<slug>:18789`, posing as `openclaw-tui` (the controlUi-bypass client ID) so the gateway accepts it without device pairing. That's a swarm-internal contract; the OpenClaw side documents it under "control UI clients" in the gateway docs.
...
sequenceDiagram
    ...
    participant Center as workspace
    ...
    Tenant->>Center: POST /api/workspace/acme/login<br/>{ email: alice@acme.de, password: ... }
...
- **Two writers to `kai-<slug>-users`**: the operator creates it empty; workspace is the only writer after that. Chat is read-only.
...
- **The JWT secret is independent of the gateway token.** Chat / workspace authenticate users with the JWT secret; the OpenClaw gateway authenticates *services* with the gateway token. They never overlap.

---

## Public platform vs. private overlay
...
A sibling **private overlay repo** (you choose the name; EmAI's own overlay is `swarm-emai`, and the SaaS-direction `.mc/` work tracks adding a separate `swarm-cloud` for hosted SaaS — see TASK-023) carries the operator-of-the-platform's overlay:
